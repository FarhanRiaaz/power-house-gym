...
class _FinancialTransactionScreenState
  ...
  Future<void> syncData() async {
    await financialStore.generateRangeReport();
  }
  ...
  void _deleteFinancialTransaction(FinancialTransaction transaction) {
    showDialog(
      ...
      builder: (ctx) => AppDialog(
        ...
        message:
            'Are you sure you want to delete Transaction ID ${transaction.id}? This action cannot be undone.',
        actions: [
          AppButton(
            label: 'Cancel',
    ...
    return Scaffold(
      ...
      body: BackgroundWrapper(
        child: Padding(
          ...
          child: Observer(
            builder: (context) {
              final totalBalance = financialStore.reportTransactionsList.fold(
                0.0,
                (sum, t) => sum + t.amount!,
              );
              ...
              return Column(
                ...
                children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            try {
                              ...
                              await financialStore.importDataToDatabase(
                                csvData,
                              );
                            } catch (e) {
                  ...
                  SizedBox(
                    ...
                    child: FinanceDateRangeFilterWidget(),
                  ),
                  const SizedBox(height: 16),
                      ...
                      child: Observer(
                        builder: (context) {
                          return financialStore.reportTransactionsList.isEmpty
                              ...
                              : ListView.builder(
                                  itemCount: financialStore
                                      .reportTransactionsList
                                      .length,
                                  physics: const ClampingScrollPhysics(),
                                  itemBuilder: (context, index) {
                                    final transaction = financialStore
                                        .reportTransactionsList[index];
                                    ...
                                    if (transaction.relatedMemberId != null) {
                                      final memberName = getMemberNameById(
                                        transaction.relatedMemberId,
                                      );
                                      primarySubtitle =
                                          'Member: ${memberName ?? 'ID ${transaction.relatedMemberId}'}';
                                    } else {
  ...
  List<FinancialTransaction> get transactions {
    ...
    financialStore.reportTransactionsList.sort(
  ...
  String? getMemberNameById(int? id) {
    ...
    return memberStore.memberList
        .firstWhereOrNull((m) => m.memberId == id)
        ?.name;
  }
}
